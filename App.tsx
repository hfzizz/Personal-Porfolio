...
`}
      ...
      case SectionType.EXPERIENCE:
        return (
          <div className="space-y-8 animate-print mt-2">
      ...
      case SectionType.PROJECTS:
        return (
          <div className="grid grid-cols-1 gap-6 mt-2 animate-print">
      ...
      case SectionType.EDUCATION:
        return (
          <div className="mt-2 border border-blue-900/30 bg-blue-950/10 p-6 rounded relative overflow-hidden animate-print">
      ...
      case SectionType.SKILLS:
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2 animate-print">
      ...
      case SectionType.CONTACT:
        return (
          <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-6 animate-print">
      ...
      setTimeout(() => {
        ...
        if (index === logs.length - 1) {
          setTimeout(() => {
            ...
            // Initialize history with Home content after boot
            setHistory([{ 
              id: 'init', 
              type: 'output', 
              content: getSectionContent(SectionType.HOME) 
            }]);
          }, 800);
        }
      }, delay);
  ...
  const handleCommand = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'Enter') {
      ...
      switch (command) {
        case 'help':
          output = (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-slate-300 animate-print mb-2">
              ...
              <div><span className="text-blue-400 font-bold">clear</span> - Clear terminal & Go Home</div>
        ...
        case 'ls':
          output = (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-blue-300 font-bold animate-print mb-2">
              {MENU_ITEMS.map(item => (
        ...
        case 'cd':
          if (!argument || argument === '~') {
            newSection = SectionType.HOME;
          } else if (argument === '..') {
            newSection = SectionType.HOME;
          } else {
            const section = MENU_ITEMS.find(item => item.id === argument || item.label.includes(argument));
            if (section) {
              newSection = section.id as SectionType;
            } else {
              output = <span className="text-red-400 mb-2">cd: no such file or directory: {argument}</span>;
            }
          }
          
          if (newSection !== activeSection || (!argument || argument === '~')) {
             setActiveSection(newSection);
             if (!output) {
                 output = getSectionContent(newSection);
             }
          }
          break;
        ...
        case 'whoami':
          output = <div className="mb-2">visitor</div>;
          break;

        case 'sudo':
          output = <div className="text-yellow-400 mb-2">user is not in the sudoers file. This incident will be reported.</div>;
          break;
        ...
        case 'cat':
          if (argument === 'resume.pdf') {
             output = <div className="mb-2">Opening resume PDF viewer... [Simulated]</div>;
          } else if (argument === 'secret.txt') {
             output = <div className="mb-2">The secret is: You are awesome!</div>;
          } else if (argument?.includes('work')) {
             newSection = SectionType.EXPERIENCE;
             setActiveSection(newSection);
             output = getSectionContent(SectionType.EXPERIENCE);
          } else if (argument?.includes('degree')) {
             newSection = SectionType.EDUCATION;
             setActiveSection(newSection);
             output = getSectionContent(SectionType.EDUCATION);
          } else if (argument?.includes('cap')) {
             newSection = SectionType.SKILLS;
             setActiveSection(newSection);
             output = getSectionContent(SectionType.SKILLS);
          } else if (argument?.includes('contact')) {
             newSection = SectionType.CONTACT;
             setActiveSection(newSection);
             output = getSectionContent(SectionType.CONTACT);
          } else {
             output = <span className="text-red-400 mb-2">cat: {argument || ''}: No such file or directory</span>;
          }
          break;

        default:
          output = <span className="text-red-400 mb-2">zsh: command not found: {command}</span>;
      }

      setHistory(prev => [
        ...prev, 
        { id: generateId(), type: 'command', path: currentPath, command: cmd },
        ...(output ? [{ id: generateId(), type: 'output' as const, content: output }] : [])
      ]);
      setInput('');
    }
  };
  ...
  const handleSidebarClick = (id: SectionType) => {
    ...
    setHistory(prev => [
       ...prev,
       { id: generateId(), type: 'output', content: getSectionContent(id) }
    ]);
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-300 font-mono selection:bg-blue-500/30 selection:text-blue-200 flex flex-col overflow-hidden">
      ...
      <div className="flex-grow p-2 sm:p-4 md:p-6 grid grid-cols-1 grid-rows-[auto_1fr] lg:grid-cols-[240px_1fr] lg:grid-rows-1 gap-4 max-w-7xl mx-auto w-full h-[calc(100vh-40px)]">
        ...
        <div className="flex flex-col border border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.15)] bg-slate-950 h-full transition-colors duration-300 min-h-0">
            ...
            >
              {history.map((item) => (
                <div key={item.id} className="mb-4">
                  {item.type === 'command' && (
                    <Prompt path={item.path} command={item.command} />
                  )}
                  {item.content && (
              ...
              {/* Active Input Line */}
              <div className="flex flex-wrap items-center gap-2 font-mono text-sm sm:text-base mt-2">
                 <div className="flex whitespace-nowrap">
                     <span className="text-green-500">[visitor@arch</span>
                     <span className="text-slate-400 mx-1"></span>
                     <span className="text-blue-400">{activeSection === SectionType.HOME ? '~' : `~/${activeSection}`}</span>
            ...
            <div className="p-2 border-t border-slate-800/50 text-slate-600 text-xs font-mono flex justify-between bg-slate-900/30">
                 <span>-- INSERT MODE --</span>
                 <span>Ln {history.length + 1}, Col {input.length + 1}</span>
            </div>
        </div>
      </div>
